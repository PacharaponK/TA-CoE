...
**Auth is a single shared secret (Phase 1).** `AdminGuard` (`backend/src/common/admin.guard.ts`) compares the `x-admin-secret` header to `ADMIN_SECRET`. GET/read endpoints are public; every mutation is guarded. On the frontend the secret lives in `localStorage` (`lib/auth.ts`); `lib/api.ts` attaches it when a call is made with `{ admin: true }`, and `components/AdminGate.tsx` gates the `/admin/*` pages by probing a protected endpoint.
