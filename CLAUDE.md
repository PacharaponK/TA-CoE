...
- `backend/.env` — `MONGODB_URI` (the DB name in the path matters; Atlas URIs without one default to `test`), `ADMIN_SECRET`, `PORT`, `CORS_ORIGIN` (comma-separated; must include the frontend origin or both REST and the Socket.io gateway will be blocked), `TELEGRAM_BOT_TOKEN` (optional; unset = Telegram notifications are a no-op).
...
**Telegram notifications on self-join.** `TelegramService` (`backend/src/telegram/`) DMs any active TA who has set a `telegramChatId` on their account (`backend/src/tas/`) whenever a student self-enqueues via `POST /queue/join` — TA-added entries do not notify. It's a fire-and-forget call (`queue.controller.ts`'s `join()` doesn't await it) so a slow/down Telegram API never blocks or fails the queue join, and it's a silent no-op if `TELEGRAM_BOT_TOKEN` is unset.
