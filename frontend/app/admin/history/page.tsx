...
function History() {
  ...
  async function handleRequeue(id: string) {
    setError('');
    try { await queueApi.requeue(id); await reload(); }
  ...
  async function handleDelete(id: string) {
    ...
    setError('');
    try { await queueApi.remove(id); await reload(); }
    catch (e) { setError((e as Error).message); }
  }
  ...
  return (
    <main className="container-page flex w-full flex-1 flex-col gap-6 py-8 relative z-10">
      <div>
        ...
        <p className="mt-1 text-sm text-zinc-400">ผลการตรวจ Checkpoint ทั้งหมด</p>
      </div>
            ...
            {grouped.map((group) => (
              ...
              >
                ...
                <div className="flex items-center gap-3 border-b border-zinc-800/70 bg-white/[0.03] px-4 py-3">
                  <span className="font-mono text-sm font-bold text-white">{group.studentId}</span>
                  <span className="text-sm text-zinc-300">{group.studentName}</span>
                  {group.section && (
                    <span className="text-xs text-zinc-500">· Sec {group.section}</span>
                  )}
                    ...
                    <div key={r._id} className="p-4">
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0 flex-1 flex flex-col gap-1">
                          ...
                          {r.checkpointName && (
                            <p className="text-xs text-zinc-400">{r.checkpointName}</p>
                          )}
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-white/5 border-b border-white/5 hover:bg-white/5">
                  {['', 'วิชา / Lab', 'Checkpoint', 'ครั้งที่', 'ผล', 'เวลาตรวจ', 'ผู้ตรวจ', ''].map((h, i) => (
                    <TableHead
                      key={i}
                  ...
                  <Fragment key={group.studentId}>
                    ...
                    {group.rows.map((r) => (
                      <TableRow key={r._id} className="border-b border-white/5 hover:bg-white/5">
                        <TableCell className="w-8 pl-5 text-zinc-600 text-xs">↳</TableCell>
                        <TableCell className="max-w-[14rem]">
                          <div className="font-medium text-white truncate" title={r.subjectName}>
                            {r.subjectName}
                          </div>
                          <div className="text-xs text-zinc-400 mt-0.5 truncate" title={r.labName}>
                            {r.labName}
                          </div>
                        </TableCell>
                        <TableCell
